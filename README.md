...
```jsonc
{
  ...
  "core": [ // Specify keys with ""'s, followed by a comma to separate each one
    " D ",
    " A ",
    " G "
  ],
...
```jsonc
{
  ...
  "requirements": { // Sets the required item for each of the ritual's item slots
    "brazier": { // The center of the ritual where an item is first burned
      ...
      // ↓ (Same as value of "type")
      "item": "minecraft:gold_ingot" // (Define a tag or item id here)
    },
  ...
  "results": [ // defines different results to occur after the ritual succeeds; there can be multiple consecutive results so be creative
    ...
    // {
    //   "type": "allure",
    //   "entity": "animal" // This can be "animal", "creature", "monster", "zombie", "villager", "skeleton", "raider", "illager", or any valid mob id
    // }
    // {
    //   "type": "absorb",
...
```jsonc
{
  ...
  "spell": {
    ...
    "transmutations": [
      {
        "ingredients": [{ "item": "eidolon:pewter_inlay" }],
        "results": [{ "item": "eidolon:unholy_symbol" }]
      },
      {
        "ingredients": [{ "item": "minecraft:black_wool" }],
        "results": [{ "item": "eidolon:top_hat" } ]
      },
      {
        "ingredients": [{ "tag": "minecraft:music_discs" }],
        "results": [{ "item": "eidolon:music_disc_parousia" }]
      }
    ])
  },
  "requirements": [
